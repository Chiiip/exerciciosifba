class CursosController < ApplicationController
  before_action :set_curso, only: [:show, :update, :edit]
  before_action :require_authentication, only: [:create, :new, :update, :destroy, :edit]
...
def buscar2
  @cursos = Curso.where(["name like :nome ", {nome: "%#{params[:search]}%"}])
end 
  ...
  private
    ...
    def curso_params
      params.require(:curso).permit(:name, :admin_id, :admin_name, :numeroalunos, :search)
    end
end
